...
class SourceMemoryStore:
    ...
    def update_ri_success(self, company_key: str, surface_url: str) -> None:
        ...
        if not u:
            return
        with self._lock:
            mem = self._load_unlocked(company_key) or CompanyMemory(company_key=company_key)
            ...
            self._save_unlocked(company_key, mem)
    ...
    def _load_unlocked(self, company_key: str) -> CompanyMemory | None:
        path = self._path(company_key)
        if not path.is_file():
            return None
        try:
    ...
    def _save_unlocked(self, company_key: str, memory: CompanyMemory) -> None:
        memory.company_key = company_key
        try:
            self._dir.mkdir(parents=True, exist_ok=True)
            path = self._path(company_key)
